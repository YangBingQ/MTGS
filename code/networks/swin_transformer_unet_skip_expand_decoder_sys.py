...
class FinalPatchExpand_X2(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=2, norm_layer=nn.LayerNorm):
        ...
        self.final = nn.Conv2d(
            in_channels=768, out_channels=256, kernel_size=1, bias=False)
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.up_global=FinalPatchExpand_X2(input_resolution=(7,7),dim_scale=2,dim=embed_dim*8)

        self.head1=nn.Sequential(nn.Conv2d(16,64,4,4),
                                nn.BatchNorm2d(64),
                                nn.ReLU(),
                                nn.Conv2d(64,64,4,4),
                                nn.BatchNorm2d(64),
        ...
        if self.final_upsample == "expand_first":
            ...
            self.head = nn.Conv2d(
                in_channels=embed_dim, out_channels=16, kernel_size=1, bias=False)
    ...
    def up_x4(self, x):
        ...
        if self.final_upsample == "expand_first":
            ...
            x = x.permute(0, 3, 1, 2)  # B,C,H,W
            local_feature=self.head(x)
            local_feature=self.head1(local_feature)
            x = self.output(x)
    ...
    def forward(self, x):
        ...
        global_feature=x_downsample[3]
        global_feature=self.up_global(global_feature)
        x = self.forward_up_features(x, x_downsample)
        x,local_feature = self.up_x4(x)

        return x,global_feature,local_feature
